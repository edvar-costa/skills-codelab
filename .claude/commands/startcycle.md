...
When the user types `/startcycle $ARGUMENTS`, orchestrate the full development lifecycle strictly using `.agents/agents.md` and `.agents/skills/`.

The `$ARGUMENTS` is the user's app idea (e.g., "API REST de tarefas com autenticação JWT").
...
- Execute the `write_specs.md` skill using the idea from `$ARGUMENTS`.
